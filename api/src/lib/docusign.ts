...
export async function healthCheck(): Promise<{ configured: boolean; jwtWorks: boolean; error?: string; hint?: string; baseUrl?: string; keyPrefix?: string }> {
  ...
  try {
    ...
    if (errorCode === 'consent_required') {
      // Use the sandbox consent host when the base URL indicates a sandbox account,
      // otherwise production. Getting this wrong produces a fresh `issuer_not_found`.
      const consentHost = (config.DOCUSIGN_BASE_URL ?? '').includes('demo')
        ? 'account-d.docusign.com'
        : 'account.docusign.com';
      hint = `You have not granted JWT consent. Visit https://${consentHost}/oauth/auth?response_type=code&scope=signature%20impersonation&client_id=${config.DOCUSIGN_INTEGRATION_KEY ?? ''}&redirect_uri=https://pub-legal-api-production.up.railway.app/api/v1/docusign/callback in a browser and click Accept.`;
